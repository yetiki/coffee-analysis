...
def yum_score(f_points, b_points, a_points, u_points):
    total_points = f_points + b_points + a_points + u_points
    return total_points
...
def main(input_csv_file: str = "data/clean/coffee_ratings.csv", output_csv_file: str = "data/clean/coffee_ratings_yscore.csv"):
    ...
    with open(input_csv_file, "r") as infile, open(output_csv_file, "w", newline="") as outfile:
        ...
        for row in reader:
            ...
            y_score = yum_score(flavour_points, body_points, aroma_points, uniformity_points)
            row.append(y_score)
            writer.writerow(row)
